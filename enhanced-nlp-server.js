...
// راه‌اندازی tokenizer برای زبان فارسی
const tokenizer = new natural.WordTokenizer();
natural.PorterStemmer.attach(); // برای stemming کلمات
...
app.get('/health', (req, res) => {
    res.json({ 
        ...
        features: ['تولید پست', 'تحلیل احساسات', 'کلمات کلیدی', 'تشخیص زبان']
    });
});
...
app.post('/api/nlp/generate-samples', (req, res) => {
    ...
    for (let i = 1; i <= 162; i++) {
        ...
        nlpPosts.push({
            ...
            content: `این پست شماره ${i} در مورد ${topic} است. موضوع ${emotion} و بسیار کاربردی می‌باشد.`,
...
app.get('/api/nlp/keyword-analysis', (req, res) => {
    ...
    tokens.forEach(token => {
        ...
        if (token.length > 3 && !['این', 'است', 'های', 'ترین'].includes(token)) {
